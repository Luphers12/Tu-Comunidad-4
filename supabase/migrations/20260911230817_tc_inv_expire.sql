CREATE OR REPLACE FUNCTION public.tc_inv_expire(
...
BEGIN
  ...
  END IF;
  IF v_reason NOT IN (
    'SOURCING_TIMEOUT', 'PAYMENT_TIMEOUT', 'RESERVATION_TIMEOUT',
    'ABANDONED_FLOW', 'OTHER_SYSTEM_TIMEOUT'
  ) THEN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'TC_EXPIRE_RACE';
  END IF;
...
REVOKE ALL ON FUNCTION public.tc_inv_expire(uuid, text, text, timestamptz) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.tc_inv_expire(uuid, text, text, timestamptz) FROM anon;
REVOKE ALL ON FUNCTION public.tc_inv_expire(uuid, text, text, timestamptz) FROM authenticated;