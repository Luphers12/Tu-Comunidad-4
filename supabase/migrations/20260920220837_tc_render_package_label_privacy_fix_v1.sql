...
BEGIN
  ...
  IF NOT v_resolved AND v_dest_id <> '' THEN
    ...
    IF FOUND THEN
      ...
      WHERE m.id = v_loc.municipality_id;

      SELECT d.name INTO v_department
      FROM public.departments d
      WHERE d.id = v_loc.department_id;
...
COMMENT ON FUNCTION public.tc_render_package_label(text)
IS 'Renders operational package-label data for authorized TIE/VEN actors without recipient identity or private delivery PII.';
