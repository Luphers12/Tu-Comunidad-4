...
alter table public.logistics_routing_attempts
  add column attempt_seq bigint generated always as identity;

create unique index logistics_routing_attempts_attempt_seq_uidx
  on public.logistics_routing_attempts(attempt_seq);

create index logistics_routing_attempts_demand_seq_idx
  on public.logistics_routing_attempts(demand_id,attempt_seq desc);
...
comment on column public.logistics_routing_attempts.attempt_seq is
'Monotonic canonical ordering for routing attempts. Use attempt_seq rather than timestamps to determine the latest attempt, including multiple attempts created within one transaction.';
